...
It is hosted on a simple Flask app: upload a flow CSV file, and the model processes it and displays the results.
When the deployment explicitly enables PCAP support and provides CICFlowMeter's Docker dependency, the same app can also accept PCAP and PCAPNG uploads.
...
`create_app(bundle_dir, enable_pcap_uploads=None)` loads one bundle directory at startup and stores it on the app.
Every `/predict` request calls `detector.detect(...)` on the uploaded CSV, so the website scores flows exactly the way `detector.py` does locally.
...
The Dockerfile now fails the build if `deploy/model_bundle/manifest.json` or `deploy/model_bundle/model.pt` are missing, so the packaged artifact is reproducible.
