...
if __name__ == "__main__":
    try:
        processed_df_for_cols = pd.read_csv("working/processed_data.csv")
        expected_features = processed_df_for_cols.drop(columns=['Label'], errors='ignore').columns.tolist()
    except FileNotFoundError:
        print("Error: processed_data.csv not found. Cannot determine expected features.")
        sys.exit(1)

    detector = AnomalyDetector(feature_columns=expected_features)

    # --- Test with a sample from your processed_data.csv ---
    print("\n--- Testing with a sample from processed_data.csv ---")
    full_data = pd.read_csv("working/processed_data.csv")
    test_sample_benign = full_data[full_data['Label'] == 'BENIGN'].drop(columns=['Label']).iloc[0:1] # Take first benign row
    test_sample_anomaly = full_data[full_data['Label'] != 'BENIGN'].drop(columns=['Label']).iloc[0:1] # Take first anomaly row

    pred_b, score_b = detector.predict(test_sample_benign)
    pred_a, score_a = detector.predict(test_sample_anomaly)

    print(f"Benign Sample Prediction: {'Anomaly' if pred_b[0] == 1 else 'Benign'} (Score: {score_b[0]:.6f})")
    print(f"Anomaly Sample Prediction: {'Anomaly' if pred_a[0] == 1 else 'Benign'} (Score: {score_a[0]:.6f})")

    # --- Test with a dummy raw input (simulating a single new flow) ---
    print("\n--- Testing with dummy raw input (structure matters!) ---")
    # This dummy data needs to match your original feature columns order and names.
    # Replace with actual feature names and dummy values.
    dummy_data = {col: [np.random.rand() * 100] for col in expected_features} # Random values
    # For a real single row input, you might construct it manually
    # Example (using actual feature names from a typical network dataset):
    # dummy_data = {
    #     'Dst Port': [80], 'Protocol': [6], 'Flow Duration': [100000],
    #     'Total Fwd Packets': [10], 'Total Backward Packets': [12],
    #     'Fwd Packet Length Max': [1000], 'Bwd Packet Length Min': [0],
    #     # ... include all your actual feature columns with appropriate values
    # }
    # Make sure to include ALL expected columns from your original dataset, even if they're 0.

    dummy_df = pd.DataFrame(dummy_data)
    dummy_df = dummy_df[expected_features]

    pred_dummy, score_dummy = detector.predict(dummy_df)
    print(f"Dummy Sample Prediction: {'Anomaly' if pred_dummy[0] == 1 else 'Benign'} (Score: {score_dummy[0]:.6f})")